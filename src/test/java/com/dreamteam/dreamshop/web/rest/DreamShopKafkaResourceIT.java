...
import com.dreamteam.dreamshop.config.EmbeddedKafka;
import com.dreamteam.dreamshop.config.KafkaSseConsumer;
import com.dreamteam.dreamshop.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class DreamShopKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/dream-shop-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/dream-shop-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/dream-shop-kafka/unregister"));
                return;
            }
        }
